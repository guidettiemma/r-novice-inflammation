...
Starting the R script with a header to inform what it contains, who wrote it and when it was created. The description should be short, clear and concise. 
...
################################################################################
```
You may also think that your R script is subject to evolve over time. So rather than creating several vintage copies (V1, V2,etc..) of it, a versionning system would be more appropriate. "Git" is the most common one and allows you to precisely track-change a script. The integration of "Git" with Rstudio enables you to view these changes in a dedicated window.
you could consider tking the following trainings:
[Software Carpentry's `git-novice` lessons][swc-lesson-git].

You may also think about not having a too long R-code and to try to modular it.
...
################################################################################
# Load the list at once and assign a TRUE if the library is already installed and
# a FALSE if not
################################################################################
lapply(list('ggplot2', 'reshape', 'vegan'), require, character.only = TRUE)
```
Second source R-script that is not part of the main but needed to run your code.
```{r source, eval=FALSE}
################################################################################
# call dependent scripts
################################################################################
source('DependentScripts.R')
```
